...
__attribute__((export_name("hex_encode"))) const char *hex_encode(const char *input)
{
    ...
    if ((p = umn_expr_parse(&lexer, &ptokens, NULL, NULL)))
    {
        if (p->token.kind & UMN_KERR)
            return NULL;

        umn_uint_t v = compute_node(&lexer, p, &err_token);

        umn_sb_pushs(&sb, "0b");
        umn_sb_pushu(&sb, v.value, 2);
    }

    return sb.items;
}

__attribute__((export_name("panic"))) void *panic()
{
    umn_panic("testing", __FILE__, __LINE__);
}
