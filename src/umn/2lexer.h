...
int umn_lexer_next(umn_Lexer *lexer, umn_Token *token)
{
  ...
  /* Would a tight loops be better or just keep this complicated mess where there is alot of weird state */
  for (; lexer->data[lexer->position] != '\0'; lexer->position++)
  {
    assert((token->kind & UMN_KERR) == 0);

    curr = umn_lexer_decode_utf8(lexer);

    /* since the logic skips leading zeroes, can safely assume that this has already figured this out */
    if ((isspace(curr) || curr <= ' ')) /* this is simply an optimisation for all of the below */
      break;

    if (token->kind == UMN_KINTEGER && !umn_lexer_read_integer(lexer, token, curr))
    {
      break; /* failed to read the integer */
    }
    else if (token->kind == UMN_KFRACTION && !umn_lexer_read_fraction(lexer, token, curr))
    {
      break;
    }
    else if (token->kind == 0 && curr == umn_Enc_String_1 || curr == umn_Enc_String_2) /* UMN_KSTRING */
    {
      ...
      /* let's highkey no be reliant on data len */
      unsigned count = 0;
